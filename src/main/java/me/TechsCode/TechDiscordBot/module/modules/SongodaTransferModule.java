...
//        TechEmbedBuilder howItWorksMessage = new TechEmbedBuilder("Transfer to SpigotMC from Songoda").text("React to the emoji below to start the transfer to SpigotMC!\n\n*Only do this if you have a Songoda account and want to transfer your plugins to SpigotMC!* **Spamming/abusing this will result in a ban!**");
//        lastInstructions = howItWorksMessage.queue(TRANSFER_CHANNEL.query().first());
...
//                    .text("Currently informing Tech about your transfer request.\n\n**Please wait...**")
...
//            new TechEmbedBuilder("Transfer to SpigotMC from Songoda (" + e.getAuthor().getName() + ")")
//                    .text("Successfully contacted Tech. You should receive the plugin on SpigotMC soon!")
...
//                        .text("Could not find any purchases linked to that email.\nWant to try again?\n\nType `cancel` to cancel.")
//                        .send(TRANSFER_CHANNEL.query().first());
//            } else {
//                selectionStep = Selection.SPIGOT_USERNAME;
//                SongodaPurchase purchase = TechDiscordBot.getSongodaPurchases().stream()
