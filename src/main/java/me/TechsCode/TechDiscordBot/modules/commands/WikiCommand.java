...
public class WikiCommand extends CommandModule {
    ...
    public void showCurrentChannel(TextChannel channel) {
        if(Plugin.isPluginChannel(channel)) {
            ...
            if(!plugin.hasWiki()) {
                new CustomEmbedBuilder("Wikis").error().setText(plugin.getEmoji().getAsMention() + " " + plugin.getRoleName() + " unfortunately does not have a wiki!").sendTemporary(channel, 10);
                return;
            }
            new CustomEmbedBuilder("Wikis").setText("*Showing the wiki of the support channel you're in.*\n\n" + plugin.getEmoji().getAsMention() + " " + plugin.getWiki() + "\n\nFor more info please execute the command `wiki help`.").send(channel);
        }
    }
